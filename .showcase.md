...
By default, `ErrShutdownAll` propagates upward. If a service inside `internalGroup` triggers a shutdown, it will also shut down `httpServer`. When that is not the behaviour you want — when a subsystem should fail in isolation — there are two ways to establish a boundary.

If you own the group definition, declare the boundary on the group itself:

```go
internalGroup := rungroup.New(rungroup.WithShutdownBoundary())
```

If you are registering a group you do not own, declare it at the registration site instead:
...
Either way, the shutdown signal is absorbed at that boundary. The root group continues running. The inner group's failure is recorded as a policy halt, not a group-wide event.
